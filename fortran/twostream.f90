...
contains
  ...
  subroutine two_stream(nz, tau, w0, u0, Rsfc, amean, surface_radiance, ierr, method)
    ...
    ierr = 0
    gt = 0.0_dp ! asymetry factor. Zero for now
    
    i = 0
    if (present(method)) i = method
    if (i == 1) then
      ! Eddington Two-Stream coefficients (Table 1)
      gam1 = (7.0_dp - w0*(4.0_dp + 3.0_dp*gt))/4.0_dp
      gam2 = -(1.0_dp- w0*(4.0_dp - 3.0_dp*gt))/4.0_dp
      gam3 = (2.0_dp - 3.0_dp*gt*u0)/4.0_dp
      gam4 = 1.0_dp - gam3
      u1 = 0.5_dp
    else ! default
    ...
    lambda = (gam1**2.0_dp - gam2**2.0_dp)**(0.5_dp)
    ...
    !   ierr = 1
    ! endif
    call tridiag(a, b, d, e) ! homebrewed version.  
